...
public class ClientApiException extends RuntimeException {
    ...
    private void deserializeProblem(RestClientResponseException cause) {
        ...
        try {
            this.problem = Optional.of(mapper.readValue(cause.getResponseBodyAsString(), Problem.class));
        } catch (JsonProcessingException ex) {
            this.problem = Optional.empty();
            log.warn("Não foi possível desserializar a resposta em un problema");
        }
    }
